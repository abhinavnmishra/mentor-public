...
const ActivityTracker = ({milestone}) => {
    ...
    const getStatusLabel = (status) => {
        switch(status) {
            case 'COMPLETED':
                return { text: 'Completed', icon: <CheckCircle sx={{ fontSize: '16px' }} /> };
            case 'ACTIVE':
                return { text: 'In Progress', icon: <AccessTime sx={{ fontSize: '16px' }} /> };
            case 'PAUSED':
                return { text: 'Not Started', icon: <Pending sx={{ fontSize: '16px' }} /> };
            default:
                return { text: 'Unknown', icon: <AccessTime sx={{ fontSize: '16px' }} /> };
        }
    };
    ...
    const handleAutoAwesomeNotes = async () => {
        ...
        try {
            const response = await axiosInstance.post('/api/ai/completion', {
                keyword: 'trainer_notes_client_activity',
                id: trackerId,
                ...
                <Grid size={{ xs: 12, md: 12 }}>
                    <SectionHeader>
                        <Box sx={{ 
                            ...
                            <Assignment sx={{ color: colors.primary, fontSize: '18px' }} />
                        </Box>
                        <Typography 
                            sx={{ 
                                fontFamily: '"Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
                    ...
                    <ModernCard>
                        <CardContent sx={{ padding: '24px' }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                                <Box sx={{ 
                                    ...
                                    <PlayArrow sx={{ color: colors.primary, fontSize: '20px' }} />
                                </Box>
                                <Box sx={{ flex: 1 }}>
                                    <Typography 
                                        variant="h6" 
                                ...
                                <StatusChip 
                                    label={getStatusLabel(milestone.status).text}
                                    status={milestone.status}
                                    icon={getStatusLabel(milestone.status).icon}
                                />
                            </Box>
                            ...
                            <Stack spacing={2}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    >
                                        <strong>Started On:</strong> {milestone.startDate ? fDate(milestone.startDate) : 'Not started'}
                                    </Typography>
                                </Box>

                                <Box sx={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <Description sx={{ color: colors.text.secondary, fontSize: '16px' }} />
                                    <Box sx={{ flex: 1 }}>
                                        ...
                                        <Collapse in={notesExpanded}>
                                            ...
                                            >
                                                {milestone.activity.notes || 'No notes available'}
                                            </Typography>
                                        </Collapse>
                                    </Box>
                                </Box>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                                    <AttachFile sx={{ color: colors.text.secondary, fontSize: '16px' }} />
                                    <Box>
                                        <Typography 
                                            sx={{ 
                                                fontFamily: '"Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
                            ...
                            <Box sx={{ marginTop: '16px', padding: '12px', backgroundColor: alpha(colors.background, 0.5), borderRadius: '8px' }}>
                                <Typography 
                                    sx={{ 
                                        fontFamily: '"Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
                                        fontSize: '0.875rem',
                        ...
                        <CardActions sx={{ padding: '16px 24px 24px 24px', gap: '12px' }}>
                            {milestone.exerciseId && (
                                <ModernButton 
                                    onClick={() => { window.open(`/portal/exercise/${milestone.exerciseId}`, '_blank');}} 
                                    variant="contained" 
                                    startIcon={<Launch />}
                                    sx={{
                            ...
                            {milestone.exerciseResponseId && (
                                <ModernButton 
                                    variant="outlined"
                                    onClick={() => { window.open(`/portal/exercise/evaluation/${milestone.exerciseResponseId}`, '_blank');}}
                        ...
                        <Box sx={{ position: 'relative', width: '100%', border: `1px solid ${colors.border}`, borderRadius: '12px', overflow: 'hidden' }}>
                            ...
                            <Editor
                                ...
                                init={{
                                    ...
                                    statusbar: true,
                                    elementpath: false,
                                    placeholder: 'Enter your trainer comments and observations about the activity...'
                                }}
                            />
                            {errors.trainerNotes && (
                                <Typography 
